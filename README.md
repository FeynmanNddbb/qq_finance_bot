...
<p>
  ...
  <img alt="APScheduler" src="https://img.shields.io/badge/scheduler-APScheduler-2C8EBB">
  <img alt="LLM" src="https://img.shields.io/badge/LLM-GLM%20%7C%20GPT%20%7C%20Claude-6E56CF">
  <img alt="NapCat" src="https://img.shields.io/badge/QQ-NapCat-00A1D6">
  <img alt="License" src="https://img.shields.io/badge/license-学习交流-lightgrey">
</p>
...
- **🤖 多模型分析（严格限定边界）** — 通过环境变量一键切换 智谱 GLM、OpenAI GPT、Anthropic Claude；模型**只对抓取到的新闻做总结与分析**，不联网、不检索、不调用工具、不编造新闻之外的内容（详见 [AI 模型的使用与边界](#-ai-模型的使用与边界重要)）
- **⏰ 定时推送** — 基于 APScheduler，默认每天 08:30 早盘、20:30 晚盘各推送一次
- **✂️ 长消息分段** — 超长消息自动分段发送，规避 QQ 单条长度限制
...
| **不编造、不臆测** | 不补充新闻中没有的事实、数字或事件；信息不足时直接说明「新闻中信息不足，无法判断」，而非强行给出结论 |
...
- Selenium 抓取部分需要本机安装 **Chrome 浏览器**及对应驱动
...
```bash
git clone https://github.com/FeynmanNddbb/qq_finance_bot.git
cd qq_finance_bot
